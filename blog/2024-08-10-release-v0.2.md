...
The new covenant-less version makes use of an embedded Bitcoin wallet, significantly simplifying deployment and management. We've also added new environment variables to configure arkd running on Bitcoin or Liquid network, offering greater flexibility for different use cases. Please note that on Bitcoin only the _covenant-less_ version is supported while on Liquid only _covenant_.
...
To enhance management capabilities, we've introduced new admin APIs that allow direct control of the onchain wallet used by the ark server.
...
While we've made significant progress, it's important to note that some features are still under development for the covenant-less version of the Ark server. These include wallet restoration and round sweeping functionalities.

Another important thing to note is that the protocol implementation is not fully trustless as originally designed yet.
Currently, the VTXO tree is signed only by the server while it should be signed also by the senders and the server misses APIs to collect such signatures. We're actively working on these aspects to ensure a robust and feature-complete implementation.
...
We're actively working on extending support for these APIs to the covenant version as well. Once this goal is achieved, we plan to deprecate the current SDK API for sending off-chain payments and promote the offline APIs from `SendAsync`, `ClaimAsynd` to `Send` and `Claim` methods.
